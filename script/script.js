...
// Tento blok získává reference na tlačítka, okno prohlížeče a horní část okna, která slouží k manipulaci.
// Také inicializuje proměnné pro sledování stavu přetahování.
...
// Tento blok nastavuje viditelnost okna pomocí kliknutí na tlačítka.
openButton.addEventListener('click', () => {
    browserWindow.style.display = 'block'; // Zobrazení okna
});

closeButton.addEventListener('click', () => {
    browserWindow.style.display = 'none'; // Skrytí okna
});

// **Manipulace s oknem (drag and drop)**
// Tento blok umožňuje přetahování okna po obrazovce pomocí myši.
// Zajišťuje také, aby okno zůstalo v rámci obrazovky.
browserTop.addEventListener('mousedown', (e) => {
    isDragging = true;
    offsetX = e.clientX - browserWindow.offsetLeft; // Výpočet relativní pozice X
    offsetY = e.clientY - browserWindow.offsetTop; // Výpočet relativní pozice Y
});

document.addEventListener('mouseup', () => {
    isDragging = false; // Deaktivace přetahování
});
...
document.addEventListener('mousemove', (e) => {
    if (isDragging) {
       ...
       // Omezení pohybu okna na obrazovku
...
// Tento blok obsahuje funkce pro zpracování vstupu uživatele a navigaci na různé stránky.
function searchQuery() {
    const query = document.getElementById('searchInput').value.trim().toLowerCase();
    navigatePage(query); // Volání navigační funkce
}
document.getElementById('searchInput').addEventListener('keydown', function(event) {
    if (event.key === 'Enter') { 
        searchQuery(); // Spuštění vyhledávání při stisknutí Enter
    }
});

// **Změna obsahu stránky**
// Tato část spravuje načítání nového obsahu na základě uživatelské volby.
function navigatePage(page) {
    const searchInput = document.getElementById('searchInput');

    searchInput.classList.add('loading'); // Zobrazení stavu načítání

    setTimeout(() => {
        searchInput.classList.remove('loading'); 
        loadPageContent(page); // Načtení obsahu po simulované prodlevě
    }, 3000); 
}
...
function toggleArticle(article) {
    const fullText = article.querySelector('.unews-article-full-text'); // Celý text článku
    const previewText = article.querySelector('.unews-article-preview'); // Náhled článku
    
    const isOpen = article.classList.contains('open'); // Kontrola, zda je článek otevřený
    
    if (isOpen) {
        // Pokud je článek otevřený, skryje jeho obsah
        fullText.style.maxHeight = '0';
        fullText.style.opacity = '0';
        previewText.style.display = 'block'; // Znovu zobrazí náhled
        article.classList.remove('open');
    } else {
        // Pokud je článek zavřený, zobrazí jeho plný obsah
        previewText.style.display = 'none'; // Skryje náhled
        fullText.style.maxHeight = '500px';
        fullText.style.opacity = '1';
        article.classList.add('open'); // Přidá stav otevření
    }
}
...
function handleClick(symbol) {
    ...
    // Pokud je sekvence delší než požadovaná, odstraní první prvek
    ...
    if (JSON.stringify(clickSequence) === JSON.stringify(requiredSequence)) {
        ...
        newText.style.display = "block"; // Zobrazí text
    }
}
...
function startExperiment() {
    ...
    section.style.display = 'block'; // Zobrazení hlavní sekce
    document.querySelectorAll('.shattered-section').forEach(sec => {
        if (sec.id !== 'section') sec.style.display = 'none'; // Skrytí ostatních sekcí
    });
}

function confirmContinue() {
    const shockingPopup = document.getElementById('shocking-popup');
    shockingPopup.style.display = 'flex';  // Zobrazení varovného popupu
}
...
function showImage() {
    ...
    `;
    shockingPopup.style.display = 'flex'; // Zobrazení popupu s obrázkem
}

// Funkce pro zavření popupu
function closePopup() {
    const shockingPopup = document.getElementById('shocking-popup');
    shockingPopup.style.display = 'none';  // Skryje popup
}
...
function showAlert() {
    const newText = document.createElement("p");
    newText.classList.add("hidden-text"); // Přidá formátování
    newText.textContent = "5 - 3aLuj11FQ42Yw2Mw";
    const introSection = document.querySelector(".intro-section");
    introSection.appendChild(newText); // Přidá text na stránku
    newText.style.display = "block";  // Zobrazí text
} 


    // Získání prvků pro poznámkový blok
    const notepadIcon = document.getElementById('icon1'); // Ikona pro otevření poznámkového bloku
    const notepadWindow = document.getElementById('notepadWindow'); // Okno poznámkového bloku
    const closeNotepad = document.getElementById('closeNotepad'); // Tlačítko pro zavření poznámkového bloku
    const notepadContent = document.getElementById('notepadContent'); // Obsah poznámkového bloku

    notepadIcon.addEventListener('click', () => {
        notepadWindow.style.display = 'block'; // Zobrazení poznámkového bloku
        notepadContent.value = localStorage.getItem('notepad') || '';  // Načtení uloženého textu z localStorage
    });

    closeNotepad.addEventListener('click', () => {
        notepadWindow.style.display = 'none'; // Skrytí poznámkového bloku
    });

    // Automatické ukládání poznámek do localStorage
    notepadContent.addEventListener('input', () => {
        localStorage.setItem('notepad', notepadContent.value); // Ukládá text poznámek do localStorage
    });

    // Funkce pro vymazání poznámek
    function clearNotepad() {
        localStorage.removeItem('notepad');  // Odstraní poznámky z localStorage
        notepadContent.value = ''; // Vymaže obsah poznámkového bloku
    }
...
<div style="display:flex; padding: 10px; flex-direction: column; justify-content: center; align-items: center; top: 20px;">
    ...
    function openKey() {
        ...
        hiddenTextElement.innerText = hiddenText2;  // Zobrazený skrytý text
        hiddenTextElement.style.display = 'block';  
    
        const browserContent = document.getElementById('browserContent');
        browserContent.appendChild(hiddenTextElement);  // Přidá text na stránku
    ...
    function inspectPage() {
        const browserContent = document.getElementById('browserContent'); // Obsah aktuální stránky
        const inspectWindow = document.getElementById('inspectWindow'); // Okno inspektoru
        const inspectContent = document.getElementById('inspectContent');  // Pole pro zobrazení kódu

        const htmlCode = browserContent.innerHTML; // Získání HTML obsahu stránky

        // Zobrazení kódu ve stylu inspektora
        inspectContent.textContent = htmlCode; // Zobrazení obsahu ve formě textu
        inspectWindow.style.display = 'block';   // Zobrazení inspektoru
    }
    ...
    function selectEmail (){
        document.getElementById("emailWindow").style.zIndex = 100; // Přenese Email okno dopředu
    ...
    function selectInspect (){
        document.getElementById("inspectWindow").style.zIndex = 100; // Přenese Inspect okno dopředu
        document.getElementById("browserWindow").style.zIndex = 1;
        document.getElementById("emailWindow").style.zIndex = 1;
        document.getElementById("notepadWindow").style.zIndex = 1;
    }
    function selectBrowser (){
        document.getElementById("browserWindow").style.zIndex = 100;  // Přenese Browser okno dopředu
        document.getElementById("emailWindow").style.zIndex = 1;
        document.getElementById("inspectWindow").style.zIndex = 1;
        document.getElementById("notepadWindow").style.zIndex = 1;
    }
    function selectNotepad (){
        document.getElementById("notepadWindow").style.zIndex = 100; // Přenese Notepad okno dopředu
        document.getElementById("browserWindow").style.zIndex = 1;
        document.getElementById("inspectWindow").style.zIndex = 1;
        document.getElementById("emailWindow").style.zIndex = 1;
    }
  ...
  popup.classList.toggle("show");
    ...
    ///MILK ROAD
            ...
            function showProductDetails(title, price, imageUrl) {
                const modal = document.getElementById('productModal'); // Modal okno pro produkt
        ...
        document.addEventListener('keydown', function(event) {
            if (event.key === 'Escape') {
                document.getElementById('gameModal').style.display = 'block'; // Zobrazení modalu při stisku klávesy Escape
            }
        });
        document.querySelector('.close').addEventListener('click', function() {
            document.getElementById('gameModal').style.display = 'none'; // Zavření modalu
        });
        function gameModalicon() {
            document.getElementById('gameModal').style.display = 'block'; // Ruční zobrazení modalu
        }
        ...
        function adjustVolume(value) {
            localStorage.setItem('volume', value); // Uložení hlasitosti do localStorage
        }
        ...
        function changeDifficulty(value) {
            ...
            localStorage.setItem('difficulty', value); // Uložení obtížnosti
            localStorage.setItem('gameDuration', gameDuration); // Uložení délky hry
        }
        
        function startGame() {
            window.location.replace("video.html"); // Přesměrování na stránku s hrou
        }
...
const emailWindow = document.getElementById('emailWindow'); // Hlavní okno aplikace
const closeEmail = document.getElementById('closeEmail'); // Tlačítko pro zavření aplikace
const emailList = document.getElementById('emailList'); // Seznam e-mailů
const emailDetail = document.getElementById('emailDetail'); // Detail zvoleného e-mailu
...
document.getElementById('openEmail').addEventListener('click', () => {
    openWindow(emailWindow); // Otevře okno aplikace
    renderEmailList();   // Načte seznam e-mailů
});

// Funkce pro otevření okna
function openWindow(window) {
    window.style.display = 'block';  // Zobrazí okno
}

// Zavření e-mailové aplikace
closeEmail.addEventListener('click', () => {
    emailWindow.style.display = 'none'; // Zavře okno aplikace
});

// Funkce pro otevření e-mailu podle ID
function openEmail(id) {
    const email = emails.find((e) => e.id === id); // Vyhledá e-mail podle ID
    if (email) {
        emailDetail.innerHTML = `
            ...
            <p>${email.content}</p>
        `;
        emailDetail.style.display = 'block';  // Zobrazí obsah e-mailu
    } else {
...
function renderEmailList() {
    ...
    emails.forEach((email) => {
        if (email.hidden) return; // Skryté e-maily se nezobrazují
        
        const listItem = document.createElement('li'); // Vytvoří položku seznamu
        listItem.innerHTML = ` 
            <strong>Odesílatel: ${email.sender}</strong>
            <p>Předmět: ${email.subject}</p>
        `;
        listItem.addEventListener('click', () => openEmail(email.id));  // Při kliknutí otevře detail
        emailList.appendChild(listItem);  // Přidá položku do seznamu
    });
}
...
renderEmailList();
        ...
        function updateGameTime() {
            ...
            if (gameTime.getHours() >= 24 || now >= gameEndTime) {
                ...
                clearInterval(gameTimeInterval);  // Konec hry
                window.location.href = 'endGametime.html'; // Přesměrování na prohru
            }
            ...
            if (hours == 22) { // Odemkne druhý e-mail v 22:00
                emails[1].hidden = false; 
                renderEmailList();        
                console.log('Email se zobrazuje, protože je přesně 22:00');
            }
            if (hours == 23) {  // Odemkne třetí e-mail v 23:00
...
function saveDrop() {
    ...
    const id = Math.random().toString(36).substring(2, 8); // Generuje náhodné ID
    localStorage.setItem(id, message); // Uloží zprávu do localStorage
    document.getElementById("dropID").innerText = id; // Zobrazí ID
    alert("Vaše zpráva byla uložena!");
}

function retrieveDrop() {
    const id = document.getElementById("retrieveID").value; // Načte ID
    const message = localStorage.getItem(id); // Načte zprávu z localStorage
    const output = message ? `Zpráva: ${message}` : "Zpráva nebyla nalezena!";
    document.getElementById("retrievedMessage").innerText = output;  // Zobrazí zprávu
}
...
function viewPublicDrop(id, message) {
    alert(`ID: ${id}\nZpráva: ${message}`); // Zobrazí obsah veřejného "Dead Drop"
}
    ...
    const hydraWindow = document.getElementById('hydraWindow'); // Okno Hydra aplikace
    const closeHydra = document.getElementById('closeHydra'); // Tlačítko zavření Hydra

    document.getElementById('openHydra').addEventListener('click', () => {
        openWindow(hydraWindow); // Otevření okna
    });
    
    function openWindow(window) {
        window.style.display = 'block'; // Zobrazení okna
    }
    
    // Zavření aplikace
    closeHydra.addEventListener('click', () => {
        hydraWindow.style.display = 'none'; // Skrytí okna
    });
    ...
    hydraTop.addEventListener('mousedown', (e) => {
        isDraggingHydra = true; // Aktivace přetahování po obrazovce
    ...
    function sendHydra() {
        if (document.getElementById('hydraContent').value === 'AbYeQ34mMFQ5WwLknpwCwCOnHZf6k5v9pj21mFw9q2ckscUTMZnbrXLF1kZp5g2k3aLuj11FQ42Yw2Mw') {
            ...
            window.location.href = 'hydra.html'; // Přesměrování na stránku Hydra
            clearNotepad(); // Vymazání poznámkového bloku
        }
    }
